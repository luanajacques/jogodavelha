...
startGame = function(){
    ...
    players[1] = {id:2, name:"O", piece:"O", score:0, isMyTurn:false, isWinner: false};
    board = [0,0,0,
             0,0,0,
             0,0,0];
    gameRoles = [[0,1,2],
...
eventHandle = function(){
    ...
    uI.btn_restart.addEventListener("click",()=>restartGame());
}
...
setMsg = function(){
    for(let player of players){
        ...
        if(player.isMyTurn && player.isWinner){
            uI.msg.innerText = "Vitória de " + player.piece;
            return 0;
        }
        if(player.isMyTurn && !player.isWinner){
            uI.msg.innerText = "Vez de " + player.piece;
            return 0;
        }
    }
}
...
setuI = function(){
    ...
    uI.btn_restart = document.getElementsByClassName("btn-restart-game")[0];
